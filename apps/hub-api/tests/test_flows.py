...
def test_list_flows(client: TestClient):
    """Test listing flows"""
    # Create a flow first
    flow_data = {"name": "Test Flow 2", "config": {}}
...
def test_get_flow(client: TestClient):
    ...
    # Create flow
    flow_data = {"name": "Test Flow 3", "config": {"test": True}}
...
def test_update_flow(client: TestClient):
    """Test updating flow"""
    # Create flow
    flow_data = {"name": "Original Flow", "config": {"version": 1}}
    create_response = client.post("/api/v1/flows/", json=flow_data)
    flow_id = create_response.json()["id"]

    # Update flow
    update_data = {"name": "Updated Flow", "config": {"version": 2}}
